...
If you just want to consult the historical demographics, there are plots and tables for the U.K. (first) and U.S. (second) in the notebook. To import the data for other purposes, use the CSV files generated by the code. Note that the CSVs contain percentage compositions by ethnicity and decade. Population counts would be easy to export from the notebook, but that isn't done at the moment.